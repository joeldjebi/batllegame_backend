 ...
 *  2. the public likes one entry per competition, judges score the entries;
 *  3. after the period the organizer publishes: the N best final scores
...
class PreselectionService
{
     ...
     *
     * @param  array{starts_at: string, ends_at: string, rules?: array<string, mixed>}  $data
     */
     ...
     */
    public function submit(Participant $participant, UploadedFile $file): PreselectionSubmission
    {
        ...
        return DB::transaction(function () use ($participant, $preselection, $file): PreselectionSubmission {
    ...
    public function rank(Preselection $preselection): void
    {
        ...
        $entries = $preselection->entries()->with(['scores', 'participant'])->where('status', PerformanceStatus::Approved)->get();
        $maxLikes = max(1, (int) $entries->max('likes_count'));

        $computed = $entries->map(function (PreselectionSubmission $entry) use ($criteria, $maxLikes, $rules) {
            ...
            $final = round(($jury ?? 0) * $rules->juryWeight / 100 + $likes * $rules->likeWeight / 100, 2);
        ...
        DB::transaction(function () use ($computed, $preselection): void {
            $preselection->entries()->where('status', '!=', PerformanceStatus::Approved)->update(['rank' => null, 'final_score' => null]);
    ...
    public function publish(Preselection $preselection): int
    {
        ...
        if ($preselection->rules->usesJury() && (clone $approved)->whereDoesntHave('scores')->exists()) {
            throw CompetitionFlowException::preselectionJuryMissing();
        }
