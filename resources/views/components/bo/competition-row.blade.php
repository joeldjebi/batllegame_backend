...
<a href="{{ route('organizers.competitions.show', [$competition->organizer, $competition]) }}"
    ...
    <span class="grid size-11 shrink-0 place-items-center rounded-xl bg-brand-50 text-brand-600 ring-1 ring-brand-100 dark:bg-brand-500/10 dark:text-brand-300 dark:ring-brand-400/20">
    ...
    <span class="hidden w-32 shrink-0 sm:block">
        ...
        <span class="mt-1.5 block h-1.5 overflow-hidden rounded-full bg-slate-100 dark:bg-white/10">
            <span class="block h-full rounded-full bg-brand-600" style="width: {{ $max ? min(100, round($count / $max * 100)) : min(100, $count * 5) }}%"></span>
        </span>
    </span>
