@props(['countries' => \App\Models\Country::query()->active()->get(), 'label' => 'Téléphone'])
...
<x-ui.field :label="$label" :error="$error" {{ $attributes }}>
    <div @class([
        ...
        <select name="country_id" required class="rounded-l-xl border-0 bg-transparent py-2.5 pr-8 pl-3 text-sm text-slate-700 focus:ring-0 dark:text-slate-200 dark:[&_option]:bg-slate-900">
        ...
        <input name="phone" value="{{ old('phone') }}" inputmode="tel" required placeholder="{{ $countries->first()?->phone_example }}"
